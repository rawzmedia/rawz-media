  ...
  const packages = [
    {
      ...
      price: "$200.000 – $600.000 + IVA",
      features: [
        "Iluminación y fondo blanco",
        "Edición básica incluida",
        "Opcional: asistente en set",
      ],
    },
    {
      name: "Producciones Corporativas (Foto + Video)",
      price: "$250.000 – $550.000 + IVA",
      features: [
        "Drone y entrevistas (opcionales)",
        "Entrega optimizada para RRSS",
        "Soporte de respaldo en set (RAWZ Tech Kit)",
      ],
    },
    {
      name: "Eventos Corporativos (Cobertura)",
      price: "$200.000 – $300.000 + IVA",
      features: [
        "Fotos + video + reel",
        "Edición incluida",
        "4 cuñas / testimonio (opcionales)",
      ],
    },
    {
      name: "Pack Evento en Santiago",
      price: "$250.000 + IVA",
      features: [
        "Fotos + reel + 4 cuñas",
        "Asistente + edición",
        "Política: 50% de anticipo",
      ],
    },
  ];
      ...
      <header className="sticky top-0 z-40 backdrop-blur supports-[backdrop-filter]:bg-zinc-950/70 border-b border-zinc-800">
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm">
            {nav.map((n) => (
              <a key={n} href={`#${n.toLowerCase()}`} className="text-zinc-300 hover:text-white transition">{n}</a>
            ))}
        ...
        <div className="mx-auto max-w-7xl px-4 pt-20 pb-16">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-3 text-4xl md:text-6xl font-extrabold tracking-tight">
              Contenido <span className="text-white/90">ágil</span>,
              <span className="text-white/90"> <span className="underline decoration-white/20">limpio</span></span> y rentable.
            </h1>
            <p className="mt-5 text-zinc-300 text-lg">
              Producimos fotos y videos que convierten: desde activaciones hasta campañas corporativas. Directo al grano, con un flujo pro y entregas listas para RRSS.
            </p>
